...
def create_product():
    ...
    if material:
        ...
        conn.execute('UPDATE MATERIAL SET quantity=?, updated_at=? WHERE material_id=?', (new_qty, now, material['material_id']))
        ...
        conn.commit()
        conn.close()
        # Sinh mã QR duy nhất cho part_code nếu chưa có file QR
    ...
    conn.commit()
    conn.close()
    # Sinh mã QR duy nhất cho lần nhập này, tên file: {part_code}.png
    qr_dir = 'static/qr_codes'
    qr_img_path = f'{qr_dir}/{part_code}.png'
    if not os.path.exists(qr_img_path):
        if not os.path.exists(qr_dir):
